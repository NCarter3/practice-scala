...
object RecursionPatternMatchingExercise {
  ...
  def checkValuesIncrease(seq: Seq[Int]): Boolean = {
    seq match {
      ...
      case first :: Nil                                                                   => true
      case first :: second :: tail if first < second && checkValuesIncrease(second::tail) => true
      case _                                                                              => false
    }
  }
  ...
  def groupConsecutive[T](in: List[T]): List[List[T]] = {
    in match {
      case in if in.isEmpty => List()
      case first :: tail => {
        val rest = groupConsecutive(tail)
        rest match {
          case (innerFirst :: innerTail) :: outerTail if first == innerFirst => (first :: innerFirst :: innerTail) :: outerTail
          case _ => List(first) :: rest
        }
      }
    }
  }
  ...
  def groupEquals[T](in: List[T]): List[List[T]] = {
    in match {
      case in if in.isEmpty => List()
      case first :: tail => in.filter((x) => (first == x)) :: groupEquals(in.filter((x) => (first != x)))
      case _ => error("default case reached")
    }
  }
  ...
  def zipMultiple(in: List[List[_]]): List[List[_]] = {
    in match {
      case in if in.isEmpty => in
      case first :: Nil => first map (List(_))
      case first :: tail => first.zip(zipMultiple(tail)) map ((x) => x._1 :: x._2)
    }
  }
